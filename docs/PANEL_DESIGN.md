...
The modern panel behavior is part of the current QEMU desktop baseline.

The latest automated baseline recorded in `CURRENT_STATE.md` passed `bash tools/verify.sh` on the v0.1 pre-reset source tree. Existing visible workflow evidence remains the manual 2026-07-17 `make qemu-fb-visible` pass; no newer manual desktop check is recorded.
...
Panel behavior also has host coverage inside `make -C tests test`, but the standalone focused runner must be invoked explicitly when changing `programs/apps/panel_model.h`.
