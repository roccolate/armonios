...
int board_storage_read(uint32_t lba, uint32_t count, void *buffer) {
    ...
    if (count != 0 && lba > UINT32_MAX - (count - 1U)) return -1;
    for (uint32_t i = 0; i < count; i++) {
        if (virtio_blk_read_sector(&g_blk_dev, lba + i,
                                   bytes + i * 512U) != 0) return -1;
    }
    return 0;
}
...
const char *board_name(void) { return "qemu-virt"; }
...
void board_early_init(void) { uart_init(QEMU_VIRT_UART0_BASE); }
void board_irq_init(void) {
    gicv2_init(QEMU_VIRT_GIC_DIST_BASE, QEMU_VIRT_GIC_CPU_BASE);
}
void board_irq_enable(uint32_t irq) { gicv2_enable_irq(irq); }
uint32_t board_irq_ack(void) { return gicv2_ack_irq(); }
void board_irq_end(uint32_t irq) { gicv2_end_irq(irq); }
int board_irq_is_spurious(uint32_t irq) { return irq == GIC_SPURIOUS_IRQ; }
uint32_t board_uart0_irq(void) { return QEMU_VIRT_UART0_IRQ; }
uint64_t board_virtio_mmio_base(void) { return QEMU_VIRT_VIRTIO_MMIO_BASE; }
uint64_t board_virtio_mmio_size(void) { return QEMU_VIRT_VIRTIO_MMIO_SIZE; }
uint64_t board_virtio_mmio_stride(void) { return QEMU_VIRT_VIRTIO_MMIO_STRIDE; }
...
int board_display_init(board_display_draw_fn_t draw, void *context) {
    ...
    g_display_ready = 0;
    if (draw == 0 ||
        virtio_gpu_probe_range(QEMU_VIRT_VIRTIO_MMIO_BASE,
                               QEMU_VIRT_VIRTIO_MMIO_SIZE,
                               QEMU_VIRT_VIRTIO_MMIO_STRIDE, &gpu_base) != 0 ||
        virtio_gpu_draw(gpu_base, draw, context) != 0) return -1;
...
int board_display_redraw(board_display_draw_fn_t draw, void *context) {
    ...
    if (draw == 0 || g_display_ready == 0U) return -1;
...
uint32_t board_input_irq(void) { return QEMU_VIRT_VIRTIO_INPUT_IRQ; }
