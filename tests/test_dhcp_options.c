...
void test_dhcp_options_parse_extracts_known_options(void) {
    ...
    TEST_ASSERT_TRUE((parsed.flags & DHCP_OPTIONS_HAS_SERVER_ID) != 0);
    ...
    TEST_ASSERT_EQUAL_UINT64(TEST_IPV4(10, 0, 2, 2), parsed.server_id);
}
...
void test_dhcp_options_parse_skips_magic_cookie_when_present(void) {
    const uint8_t options[] = {
        0x63, 0x82, 0x53, 0x63,
        53, 1, DHCP_OFFER,
        54, 4, 10, 0, 2, 2,
...
void test_dhcp_options_parse_clears_output_on_error(void) {
    ...
    TEST_ASSERT_EQUAL_UINT64(0, parsed.server_id);
}
