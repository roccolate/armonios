#include "unity/unity.h"

/* Declare test functions */
void test_pmm_init_alloc_free_count(void);
void test_pmm_reserve_range(void);
void test_pmm_reserve_range_clamps_overlap_before_base(void);
void test_pmm_reserve_range_handles_overflowing_end(void);
void test_pmm_alloc_pages_requires_contiguous_run(void);
void test_kheap_basic_alloc_free(void);
void test_kheap_rejects_empty_and_impossible_allocations(void);
void test_kheap_allocates_larger_than_one_page(void);
void test_kheap_allocates_panel_sized_backing_buffer(void);
void test_kheap_reuses_large_freed_block_for_small_allocations(void);
void test_kheap_large_alloc_free_cycles_remain_usable(void);
void test_vmm_map_translate_unmap_page(void);
void test_vmm_map_range_and_unmap_range(void);
void test_vmm_map_range_rejects_overflowing_span(void);
void test_vmm_map_range_rolls_back_partial_failure(void);
void test_vmm_unmap_range_rejects_overflowing_span(void);
void test_vmm_user_exec_mapping_flags(void);
void test_vmm_kernel_mapping_can_be_replaced_with_user_mapping(void);
void test_vmm_free_table_releases_tables_not_leaf_pages(void);
void test_process_user_range_contains_registered_regions(void);
void test_process_remove_user_region_exact_match(void);
void test_process_user_region_mapping_metadata_round_trips(void);
void test_process_user_region_helpers_reject_overflowing_ranges(void);
void test_process_alloc_user_region_bumps_page_aligned_arena(void);
void test_process_alloc_user_region_respects_region_limit(void);
void test_process_current_and_region_limits(void);
void test_process_entry_page_table_and_exit_state(void);
void test_process_save_context_copies_registers_and_trap_state(void);
void test_process_load_context_writes_exception_frame(void);
void test_process_table_alloc_release_and_limits(void);
void test_process_at_returns_active_slots_only(void);
void test_process_wait_zombie_returns_exit_and_releases_slot(void);
void test_process_kill_marks_target_zombie(void);
void test_process_next_runnable_round_robin_and_reclaim_zombies(void);
void test_process_dispatch_next_preempt_marks_current_ready(void);
void test_process_dispatch_next_exit_leaves_current_zombie(void);
void test_process_free_resources_releases_owned_pages(void);
void test_process_free_resources_is_idempotent(void);
void test_process_release_releases_resources(void);
void test_process_mark_exited_is_idempotent_in_state(void);
void test_init_status_defaults_to_skipped(void);
void test_init_status_set_and_get_round_trip(void);
void test_init_status_rejects_invalid_phase(void);
void test_init_status_labels_are_stable(void);
void test_gui_destroy_windows_for_pid_destroys_owner_windows(void);
void test_gui_destroy_windows_for_pid_is_safe_on_no_match(void);
void test_gui_destroy_windows_for_pid_handles_null_and_no_owner_sentinel(void);
void test_boot_program_find_existing_program(void);
void test_boot_program_find_rejects_missing_and_invalid_names(void);
void test_boot_program_metadata_round_trips_image_range(void);
void test_boot_program_find_resolves_editor_and_panel(void);
void test_boot_program_find_resolves_remaining_registered_apps(void);
void test_boot_program_image_points_to_flat_header_magic(void);
void test_boot_program_registered_apps_have_canonical_kli1_headers(void);
void test_boot_program_editor_image_size_matches_assembly_blob(void);
void test_boot_program_find_results_survive_later_lookups(void);
void test_panel_boot_argv_rejects_too_many_strings(void);
void test_panel_boot_argv_rejects_total_string_budget_overflow(void);
void test_panel_boot_argv_packs_well_formed_args_with_alignment_and_sentinel(void);
void test_panel_boot_argv_zero_argc_returns_null_argv(void);
void test_panel_boot_argv_rejects_invalid_stack_inputs(void);
void test_panel_boot_recovery_max_attempts_constant_is_sane(void);
void test_panel_boot_recovery_decide_first_attempt_returns_continue(void);
void test_panel_boot_recovery_decide_middle_attempt_returns_continue(void);
void test_panel_boot_recovery_decide_max_attempts_returns_stop(void);
void test_panel_boot_recovery_decide_overshoot_returns_stop(void);
void test_panel_boot_recovery_decide_zero_attempts_returns_continue(void);
void test_panel_boot_recovery_wrapper_stops_after_budget(void);
void test_panel_boot_recovery_wrapper_propagates_last_exit_code(void);
void test_panel_boot_recovery_run_rejects_null_callback(void);
void test_irq_register_rejects_invalid_inputs(void);
void test_irq_dispatches_registered_handler_and_sends_eoi(void);
void test_irq_unregister_prevents_dispatch_but_still_sends_eoi(void);
void test_irq_spurious_skips_handler_and_eoi(void);
void test_sched_timer_quantum_counts_elapsed_ticks(void);
void test_sched_zero_quantum_defaults_to_one_tick(void);
void test_sched_start_without_threads_does_not_switch(void);
void test_sched_yield_without_current_thread_is_noop(void);
void test_bootfs_find_existing_file_metadata(void);
void test_bootfs_find_returns_each_registered_app(void);
void test_bootfs_find_rejects_missing_and_invalid_names(void);
void test_bootfs_finds_editor_and_panel_for_taskbar_spawn(void);
void test_bootfs_finds_monitor_and_clock(void);
void test_bootfs_read_copies_requested_range(void);
void test_bootfs_read_clamps_at_end_of_file(void);
void test_bootfs_read_rejects_invalid_inputs(void);
void test_bootfs_mount_vfs_exposes_armonios_paths(void);
void test_bootfs_mount_vfs_exposes_editor_and_panel_paths(void);
void test_bootfs_mount_vfs_exposes_remaining_app_paths(void);
void test_bootfs_mount_vfs_reads_app_through_vfs(void);
void test_bootfs_mount_vfs_reads_editor_through_vfs(void);
void test_bootfs_old_user_demo_path_is_gone(void);
void test_bootfs_find_results_survive_later_lookups(void);
void test_user_image_entry_uses_base_plus_offset(void);
void test_user_image_prepare_process_registers_image_and_stack(void);
void test_user_image_load_copy_copies_source_and_sets_descriptor(void);
void test_user_image_load_flat_uses_header_entry_table(void);
void test_user_image_load_bootfs_flat_uses_named_boot_file(void);
void test_user_image_prepare_process_rejects_invalid_inputs(void);
void test_user_image_load_copy_rejects_invalid_ranges(void);
void test_user_image_load_flat_rejects_invalid_headers(void);
void test_user_image_load_flat_rejects_noncanonical_header_size(void);
void test_user_image_load_bootfs_flat_rejects_missing_file(void);
void test_user_image_runtime_well_formed_covers_rodata_at_tail(void);
void test_user_image_runtime_short_image_size_drops_tail(void);
void test_user_image_runtime_rejects_image_size_zero(void);
void test_user_image_runtime_rejects_entry_outside_range(void);
void test_user_image_runtime_entry_offset_matches_image_base_plus_offset(void);
void test_user_image_runtime_shipping_shell_image_size(void);
void test_user_image_runtime_shipping_editor_image_size(void);
void test_user_image_runtime_shipping_files_image_size(void);
void test_user_image_runtime_shipping_monitor_image_size(void);
void test_user_image_runtime_shipping_clock_image_size(void);
void test_user_image_runtime_shipping_panel_image_size(void);
void test_user_image_runtime_shipping_control_image_size(void);
void test_user_vm_map_anonymous_installs_user_pte(void);
void test_user_vm_unmap_anonymous_removes_pte_and_frees_pages(void);
void test_user_vm_rejects_invalid_inputs_without_regions(void);
void test_user_vm_map_physical_maps_registered_region(void);
void test_user_vm_map_physical_rejects_unregistered_region(void);
void test_user_vm_physical_mappings_stay_per_process(void);
void test_vfs_mount_and_find_static_node(void);
void test_vfs_read_dispatches_to_node_reader(void);
void test_vfs_rejects_invalid_mounts_and_reads(void);
void test_vfs_mount_static_respects_node_limit(void);
void test_vfs_strip_prefix_returns_suffix_for_exact_prefix(void);
void test_vfs_strip_prefix_rejects_invalid_or_empty_suffix(void);
void test_vfs_open_read_fd_and_close(void);
void test_vfs_open_flags_enforce_read_write_modes(void);
void test_vfs_open_rejects_missing_and_invalid_descriptors(void);
void test_vfs_open_respects_descriptor_limit_and_reuses_closed_slot(void);
void test_vfs_stat_reports_node_size(void);
void test_vfs_list_root_returns_mounted_paths(void);
void test_vfs_list_truncates_to_capacity(void);
void test_vfs_mount_list_validates_paths_and_duplicates(void);
void test_vfs_seek_sets_descriptor_offset(void);
void test_vfs_write_dispatches_to_node_writer(void);
void test_vfs_open_write_fd_updates_offset_and_size(void);
void test_vfs_rejects_overreported_io_counts(void);
void test_vfs_unlink_rejects_non_fat_paths(void);
void test_vfs_rename_rejects_non_fat_paths(void);
void test_tmpfs_create_write_read_stat_and_delete(void);
void test_tmpfs_write_clamps_to_file_capacity(void);
void test_tmpfs_reused_slot_clears_old_file_bytes(void);
void test_tmpfs_rejects_invalid_inputs_and_duplicates(void);
void test_tmpfs_respects_file_limit_and_reuses_deleted_slot(void);
void test_tmpfs_mount_vfs_read_write_and_stat(void);
void test_tmpfs_mount_vfs_rejects_missing_and_invalid_inputs(void);
void test_tmpfs_failed_mount_does_not_consume_slot(void);
void test_fat32_mount_reads_geometry(void);
void test_fat32_default_fs_tracks_successful_mount_only(void);
void test_fat32_open_root_finds_8_3_file(void);
void test_fat32_read_file_across_cluster_chain(void);
void test_fat32_read_clamps_and_honors_offset(void);
void test_fat32_rejects_invalid_boot_sector_and_missing_files(void);
void test_fat32_mount_vfs_file_reads_through_vfs(void);
void test_fat32_list_root_returns_short_names(void);
void test_fat32_mount_vfs_root_lists_through_vfs(void);
void test_fat32_write_overwrites_and_updates_size(void);
void test_fat32_write_grows_within_chain_and_rejects_capacity_overflow(void);
void test_fat32_mount_vfs_file_writes_through_vfs(void);
void test_fat32_vfs_open_mounts_existing_root_file_dynamically(void);
void test_fat32_vfs_open_create_makes_missing_root_file(void);
void test_fat32_vfs_open_create_rejects_bad_flags_and_paths(void);
void test_fat32_create_writes_dir_entry_and_allocates_cluster(void);
void test_fat32_create_then_write_persists_through_open(void);
void test_fat32_create_rejects_duplicate_name(void);
void test_fat32_write_grows_chain_when_capacity_exhausted(void);
void test_fat32_delete_frees_chain_and_removes_entry(void);
void test_fat32_delete_allows_empty_file_without_cluster(void);
void test_fat32_rename_updates_short_name_in_place(void);
void test_fat32_rename_rejects_destination_collision(void);
void test_fat32_write_to_brand_new_file_uses_allocated_cluster(void);
void test_dhcp_options_parse_extracts_known_options(void);
void test_dhcp_options_parse_skips_padding_and_stops_at_end(void);
void test_dhcp_options_parse_skips_magic_cookie_when_present(void);
void test_dhcp_options_parse_rejects_truncated_option(void);
void test_dhcp_options_parse_clears_output_on_error(void);
void test_ipc_send_and_recv_message(void);
void test_ipc_keeps_messages_by_target_pid(void);
void test_ipc_preserves_fifo_after_slot_reuse(void);
void test_ipc_recv_zeroes_unused_payload_tail(void);
void test_ipc_rejects_invalid_inputs_and_full_queue(void);
void test_font_draw_char_renders_8x8_glyph(void);
void test_font_draw_text_advances_and_handles_lowercase(void);
void test_font_draw_text_renders_punctuation(void);
void test_font_draw_char_ignores_non_ascii_bytes(void);
void test_font_draw_text_clips_at_framebuffer_edge(void);
void test_font_draw_text_handles_newline(void);
void test_font_draw_text_clipped_stops_at_max_h(void);
void test_font_draw_text_clipped_handles_zero_max_h(void);
void test_gui_init_for_framebuffer_starts_with_empty_desktop(void);
void test_gui_create_draws_windows_in_creation_order(void);
void test_gui_move_window_redraws_at_new_position(void);
void test_gui_rejects_invalid_inputs_and_window_limit(void);
void test_gui_delivers_key_to_focused_window(void);
void test_gui_cursor_move_clamps_to_bounds(void);
void test_gui_draw_renders_16x16_cursor_over_desktop(void);
void test_gui_left_click_raises_window_under_cursor(void);
void test_gui_left_click_on_background_clears_focus(void);
void test_gui_right_and_middle_buttons_set_mask_without_focus_change(void);
void test_gui_focused_window_gets_brighter_border(void);
void test_gui_drag_moves_window_following_cursor(void);
void test_gui_drag_rejects_unknown_window(void);
void test_gui_drag_stays_active_until_left_release(void);
void test_gui_title_bar_shifts_owner_draw_below_bar(void);
void test_gui_title_bar_paints_bar_and_text(void);
void test_gui_title_separator_stays_inside_title_bar(void);
void test_gui_partial_repaint_preserves_title_close_button(void);
void test_gui_cursor_damage_restores_partial_close_icon(void);
void test_gui_partial_title_repaint_does_not_bleed_over_front_window(void);
void test_gui_partial_title_text_repaint_is_clipped_to_damage_rect(void);
void test_gui_cursor_shape_changes_over_clickable_title_decoration(void);
void test_gui_set_cursor_shape_rejects_unknown_shapes(void);
void test_gui_cursor_register_region_installs_shape_inside_window(void);
void test_gui_cursor_register_region_overrides_title_bar_default(void);
void test_gui_cursor_register_region_first_matching_slot_wins(void);
void test_gui_cursor_register_region_delete_clears_slot(void);
void test_gui_cursor_register_region_rejects_invalid_inputs(void);
void test_gui_cursor_register_region_clears_on_window_destroy(void);
void test_gui_cursor_register_region_constant_is_sane(void);
void test_gui_set_title_bar_rejects_height_larger_than_window(void);
void test_gui_set_title_bar_zero_disables_bar(void);
void test_gui_decoration_hit_test_reports_slot_geometry(void);
void test_gui_close_button_pushed_event_on_title_bar_click(void);
void test_gui_close_button_destroys_window_for_dead_owner(void);
void test_gui_close_button_destroys_window_for_reclaimed_owner(void);
void test_gui_close_button_click_outside_box_starts_drag(void);
void test_gui_focus_window_switches_to_target(void);
void test_gui_focus_window_raises_target_above_overlap(void);
void test_gui_window_for_pid_returns_owner_windows(void);
void test_gui_backing_buffer_lazy_allocates_on_first_draw(void);
void test_gui_backing_buffer_init_to_bg_color(void);
void test_gui_backing_buffer_survives_compositor_redraw(void);
void test_gui_backing_buffer_follows_window_on_drag(void);
void test_gui_backing_buffer_freed_on_destroy(void);
void test_gui_backing_buffer_reallocates_when_window_reused(void);
void test_gui_backing_buffer_resize_clears_reused_capacity(void);
void test_gui_damage_init_queues_full_sentinel(void);
void test_gui_damage_add_clips_to_framebuffer(void);
void test_gui_damage_add_merges_overlapping_rects(void);
void test_gui_damage_add_full_collapses_to_sentinel(void);
void test_gui_damage_overflow_collapses_to_sentinel(void);
void test_gui_damage_draw_text_pushes_tight_rect(void);
void test_gui_owner_draw_marks_desktop_dirty_without_flush(void);
void test_gui_damage_draw_rect_pushes_content_rect(void);
void test_gui_damage_title_bar_offset_in_draw_text(void);
void test_gui_damage_move_pushes_union_rect(void);
void test_gui_damage_partial_repaint_leaves_pixels_outside_rect(void);
void test_gui_damage_full_sentinel_re_paints_everything(void);
void test_gui_damage_partial_repaint_paints_every_pixel_in_rect(void);
void test_gui_damage_partial_repaint_skips_when_rect_outside_fb(void);
void test_gui_damage_partial_repaint_with_multiple_disjoint_rects(void);
void test_gui_partial_window_repaint_does_not_overwrite_cursor_outside_damage(void);
void test_gui_minimized_windows_are_not_composited(void);
void test_gui_cursor_is_painted_after_full_window_composition(void);
void test_gui_cursor_is_painted_after_partial_window_composition(void);
void test_input_injects_ascii_byte_directly(void);
void test_input_translates_esc_up_to_key_up(void);
void test_input_translates_esc_down_to_key_down(void);
void test_input_translates_esc_left_and_right(void);
void test_input_keeps_esc_for_unrelated_followup(void);
void test_input_supports_consecutive_esc_sequences(void);
void test_input_translates_ansi_navigation_tilde_sequences(void);
void test_input_keymap_maps_navigation_keys(void);
void test_input_keymap_maps_ctrl_letters(void);
void test_input_queue_init_resets_partial_ansi_sequence(void);
void test_input_poll_char_mask_strips_high_bits(void);
void test_input_peek_returns_event_without_removing(void);
void test_input_peek_returns_minus_one_when_empty(void);
void test_input_peek_then_poll_returns_same_event(void);
void test_virtio_input_probe_rejects_wrong_device(void);
void test_virtio_input_init_records_negotiated_queue_size(void);
void test_virtio_input_rejects_invalid_inputs(void);
void test_virtio_input_key_translation_maps_ascii_and_navigation(void);
void test_virtio_input_key_translation_maps_ctrl_letters(void);
void test_hid_parse_boot_keyboard_decodes_modifiers_and_keys(void);
void test_hid_parse_boot_keyboard_rejects_short_buffer(void);
void test_hid_parse_boot_keyboard_rejects_null(void);
void test_hid_parse_boot_mouse_decodes_buttons_and_motion(void);
void test_hid_parse_boot_mouse_sign_extends_negative_motion(void);
void test_hid_parse_boot_mouse_rejects_short_buffer(void);
void test_hid_usage_to_ascii_lowercase_letters(void);
void test_hid_usage_to_ascii_uppercase_letters_with_shift(void);
void test_hid_usage_to_ascii_digits_and_punctuation(void);
void test_hid_usage_to_ascii_shifted_punctuation(void);
void test_hid_usage_to_ascii_returns_zero_for_modifiers(void);
void test_hid_usage_to_ascii_returns_zero_for_function_keys(void);
void test_hid_usage_to_ascii_high_usage_ids_return_zero(void);
void test_hid_usage_to_input_key_maps_navigation(void);
void test_hid_usage_to_input_key_maps_ctrl_letters(void);
void test_hid_keyboard_report_emits_press_for_new_key(void);
void test_hid_keyboard_report_emits_navigation_key(void);
void test_hid_keyboard_report_emits_ctrl_s(void);
void test_hid_keyboard_report_emits_release_when_key_dropped(void);
void test_hid_keyboard_report_filters_single_empty_repeat_gap(void);
void test_hid_keyboard_report_handles_two_key_rollover(void);
void test_hid_mouse_report_emits_move_when_deltas_nonzero(void);
void test_hid_mouse_report_emits_button_press_and_release(void);
void test_hid_mouse_report_emits_no_events_when_idle(void);
void test_hid_mouse_report_rejects_null_arguments(void);
void test_pci_bar_address_extracts_32bit_mmio(void);
void test_pci_bar_address_ignores_io_space_bar(void);
void test_pci_bar_address_combines_64bit_pair(void);
void test_pci_bar_address_low_32bit_ignores_next_bar(void);
void test_usb_walk_configuration_finds_interface(void);
void test_usb_walk_configuration_attaches_endpoint(void);
void test_usb_walk_configuration_attaches_hid_class_descriptor(void);
void test_usb_walk_configuration_rejects_short_buffer(void);
void test_usb_walk_configuration_rejects_wrong_descriptor_type(void);
void test_usb_walk_configuration_rejects_short_typed_descriptors(void);
void test_usb_find_interface_returns_null_when_no_match(void);
void test_usb_find_interface_matches_exact_class_subclass_protocol(void);
void test_usb_find_endpoint_in_returns_interrupt_endpoint(void);
void test_usb_find_endpoint_in_returns_null_when_too_small(void);
void test_pci_assign_bars_skips_already_programmed_bars(void);
void test_pci_assign_bars_function_pointer_exists(void);
void test_hid_state_reset_clears_count(void);
void test_hid_state_reset_clears_device_endpoints(void);
void test_hid_poll_all_returns_zero_on_empty_state(void);
void test_hid_state_count_field_is_accessible(void);
void test_hid_max_devices_constant_is_sane(void);
void test_hid_init_registers_mouse_protocol(void);
void test_hid_init_zeroes_prev_keys_for_new_device(void);
void test_hid_add_device_appends_and_copies_usb_handle(void);
void test_hid_init_returns_zero_when_no_boot_interfaces(void);
void test_hid_init_returns_zero_for_null_state(void);
void test_hid_init_keeps_max_four_devices(void);
void test_gui_draw_fills_vertical_gradient(void);
void test_fb_init_rejects_invalid_geometry(void);
void test_fb_putpixel_and_fillrect_clip_to_bounds(void);
void test_fb_blit_copies_and_clips_source_pixels(void);
void test_fb_draw_line_rect_circle_and_alpha(void);
void test_fb_draw_rect_clips_huge_dimensions_without_wrapping(void);
void test_dtb_get_framebuffer_reads_simple_framebuffer(void);
void test_dtb_get_framebuffer_rejects_incomplete_node(void);
void test_dtb_get_framebuffer_rejects_header_offsets_outside_blob(void);
void test_virtio_blk_probe_reads_capacity(void);
void test_virtio_blk_probe_rejects_non_block_device(void);
void test_virtio_blk_probe_range_finds_later_transport(void);
void test_virtio_blk_init_programs_modern_queue(void);
void test_virtio_net_probe_reads_mac_address(void);
void test_virtio_net_init_negotiates_feature_banks(void);
void test_virtio_net_tx_uses_single_frame_buffer(void);
void test_virtio_net_ready_ops_reject_invalid_device_state(void);
void test_syscall_abi_implemented_numbers_match_dispatch(void);
void test_syscall_abi_ranges_do_not_overlap(void);
void test_syscall_abi_error_codes_match_documented_constants(void);
void test_syscall_abi_vfs_open_flags_match_documentation(void);
void test_syscall_abi_user_exit_codes_match_documented_constants(void);
void test_syscall_abi_user_range_validation_rejects_out_of_region(void);
void test_syscall_helpers_user_buffers_validate_registered_ranges(void);
void test_syscall_helpers_copy_cstr_validates_each_byte(void);
void test_syscall_helpers_copy_cstr_rejects_unregistered_tail(void);
void test_syscall_helpers_owner_window_error_modes_are_stable(void);
void test_process_isolation_two_processes_have_independent_regions(void);
void test_process_isolation_mmap_returns_distinct_addresses(void);
void test_process_isolation_mmap_rejects_hint_nonzero(void);
void test_process_isolation_mmap_rejects_unknown_flags(void);
void test_process_isolation_zero_process_returns_false(void);
void test_window_abi_event_layout_is_12_bytes(void);
void test_window_abi_event_types_match_documented_numbers(void);
void test_window_abi_create_window_for_pid_assigns_owner(void);
void test_window_abi_focus_window_raises_z_order(void);
void test_window_abi_focus_no_focus_window_rejected(void);
void test_window_abi_window_for_pid_skips_no_owner(void);
void test_window_abi_window_for_pid_skips_skip_taskbar(void);
void test_window_abi_get_bounds_returns_current_geometry(void);
void test_window_abi_resize_window_updates_geometry_and_queues_event(void);
void test_window_abi_resize_window_rejects_out_of_bounds(void);
void test_window_abi_event_types_includes_minimize_and_maximize(void);
void test_window_abi_minimize_hides_window_and_queues_event(void);

int main(void) {
    UNITY_BEGIN();

    RUN_TEST(test_pmm_init_alloc_free_count);
    RUN_TEST(test_pmm_reserve_range);
    RUN_TEST(test_pmm_reserve_range_clamps_overlap_before_base);
    RUN_TEST(test_pmm_reserve_range_handles_overflowing_end);
    RUN_TEST(test_pmm_alloc_pages_requires_contiguous_run);
    RUN_TEST(test_kheap_basic_alloc_free);
    RUN_TEST(test_kheap_rejects_empty_and_impossible_allocations);
    RUN_TEST(test_kheap_allocates_larger_than_one_page);
    RUN_TEST(test_kheap_allocates_panel_sized_backing_buffer);
    RUN_TEST(test_kheap_reuses_large_freed_block_for_small_allocations);
    RUN_TEST(test_kheap_large_alloc_free_cycles_remain_usable);
    RUN_TEST(test_vmm_map_translate_unmap_page);
    RUN_TEST(test_vmm_map_range_and_unmap_range);
    RUN_TEST(test_vmm_map_range_rejects_overflowing_span);
    RUN_TEST(test_vmm_map_range_rolls_back_partial_failure);
    RUN_TEST(test_vmm_unmap_range_rejects_overflowing_span);
    RUN_TEST(test_vmm_user_exec_mapping_flags);
    RUN_TEST(test_vmm_kernel_mapping_can_be_replaced_with_user_mapping);
    RUN_TEST(test_vmm_free_table_releases_tables_not_leaf_pages);
    RUN_TEST(test_process_user_range_contains_registered_regions);
    RUN_TEST(test_process_remove_user_region_exact_match);
    RUN_TEST(test_process_user_region_mapping_metadata_round_trips);
    RUN_TEST(test_process_user_region_helpers_reject_overflowing_ranges);
    RUN_TEST(test_process_alloc_user_region_bumps_page_aligned_arena);
    RUN_TEST(test_process_alloc_user_region_respects_region_limit);
    RUN_TEST(test_process_current_and_region_limits);
    RUN_TEST(test_process_entry_page_table_and_exit_state);
    RUN_TEST(test_process_save_context_copies_registers_and_trap_state);
    RUN_TEST(test_process_load_context_writes_exception_frame);
    RUN_TEST(test_process_table_alloc_release_and_limits);
    RUN_TEST(test_process_at_returns_active_slots_only);
    RUN_TEST(test_process_wait_zombie_returns_exit_and_releases_slot);
    RUN_TEST(test_process_kill_marks_target_zombie);
    RUN_TEST(test_process_next_runnable_round_robin_and_reclaim_zombies);
    RUN_TEST(test_process_dispatch_next_preempt_marks_current_ready);
    RUN_TEST(test_process_dispatch_next_exit_leaves_current_zombie);
    RUN_TEST(test_process_free_resources_releases_owned_pages);
    RUN_TEST(test_process_free_resources_is_idempotent);
    RUN_TEST(test_process_release_releases_resources);
    RUN_TEST(test_process_mark_exited_is_idempotent_in_state);
    RUN_TEST(test_init_status_defaults_to_skipped);
    RUN_TEST(test_init_status_set_and_get_round_trip);
    RUN_TEST(test_init_status_rejects_invalid_phase);
    RUN_TEST(test_init_status_labels_are_stable);
    RUN_TEST(test_gui_destroy_windows_for_pid_destroys_owner_windows);
    RUN_TEST(test_gui_destroy_windows_for_pid_is_safe_on_no_match);
    RUN_TEST(test_gui_destroy_windows_for_pid_handles_null_and_no_owner_sentinel);
    RUN_TEST(test_boot_program_find_existing_program);
    RUN_TEST(test_boot_program_find_rejects_missing_and_invalid_names);
    RUN_TEST(test_boot_program_metadata_round_trips_image_range);
    RUN_TEST(test_boot_program_find_resolves_editor_and_panel);
    RUN_TEST(test_boot_program_find_resolves_remaining_registered_apps);
    RUN_TEST(test_boot_program_image_points_to_flat_header_magic);
    RUN_TEST(test_boot_program_registered_apps_have_canonical_kli1_headers);
    RUN_TEST(test_boot_program_editor_image_size_matches_assembly_blob);
    RUN_TEST(test_boot_program_find_results_survive_later_lookups);
    RUN_TEST(test_panel_boot_argv_rejects_too_many_strings);
    RUN_TEST(test_panel_boot_argv_rejects_total_string_budget_overflow);
    RUN_TEST(test_panel_boot_argv_packs_well_formed_args_with_alignment_and_sentinel);
    RUN_TEST(test_panel_boot_argv_zero_argc_returns_null_argv);
    RUN_TEST(test_panel_boot_argv_rejects_invalid_stack_inputs);
    RUN_TEST(test_panel_boot_recovery_max_attempts_constant_is_sane);
    RUN_TEST(test_panel_boot_recovery_decide_first_attempt_returns_continue);
    RUN_TEST(test_panel_boot_recovery_decide_middle_attempt_returns_continue);
    RUN_TEST(test_panel_boot_recovery_decide_max_attempts_returns_stop);
    RUN_TEST(test_panel_boot_recovery_decide_overshoot_returns_stop);
    RUN_TEST(test_panel_boot_recovery_decide_zero_attempts_returns_continue);
    RUN_TEST(test_panel_boot_recovery_wrapper_stops_after_budget);
    RUN_TEST(test_panel_boot_recovery_wrapper_propagates_last_exit_code);
    RUN_TEST(test_panel_boot_recovery_run_rejects_null_callback);
    RUN_TEST(test_irq_register_rejects_invalid_inputs);
    RUN_TEST(test_irq_dispatches_registered_handler_and_sends_eoi);
    RUN_TEST(test_irq_unregister_prevents_dispatch_but_still_sends_eoi);
    RUN_TEST(test_irq_spurious_skips_handler_and_eoi);
    RUN_TEST(test_sched_timer_quantum_counts_elapsed_ticks);
    RUN_TEST(test_sched_zero_quantum_defaults_to_one_tick);
    RUN_TEST(test_sched_start_without_threads_does_not_switch);
    RUN_TEST(test_sched_yield_without_current_thread_is_noop);
    RUN_TEST(test_bootfs_find_existing_file_metadata);
    RUN_TEST(test_bootfs_find_returns_each_registered_app);
    RUN_TEST(test_bootfs_find_rejects_missing_and_invalid_names);
    RUN_TEST(test_bootfs_finds_editor_and_panel_for_taskbar_spawn);
    RUN_TEST(test_bootfs_finds_monitor_and_clock);
    RUN_TEST(test_bootfs_read_copies_requested_range);
    RUN_TEST(test_bootfs_read_clamps_at_end_of_file);
    RUN_TEST(test_bootfs_read_rejects_invalid_inputs);
    RUN_TEST(test_bootfs_mount_vfs_exposes_armonios_paths);
    RUN_TEST(test_bootfs_mount_vfs_exposes_editor_and_panel_paths);
    RUN_TEST(test_bootfs_mount_vfs_exposes_remaining_app_paths);
    RUN_TEST(test_bootfs_mount_vfs_reads_app_through_vfs);
    RUN_TEST(test_bootfs_mount_vfs_reads_editor_through_vfs);
    RUN_TEST(test_bootfs_old_user_demo_path_is_gone);
    RUN_TEST(test_bootfs_find_results_survive_later_lookups);
    RUN_TEST(test_user_image_entry_uses_base_plus_offset);
    RUN_TEST(test_user_image_prepare_process_registers_image_and_stack);
    RUN_TEST(test_user_image_load_copy_copies_source_and_sets_descriptor);
    RUN_TEST(test_user_image_load_flat_uses_header_entry_table);
    RUN_TEST(test_user_image_load_bootfs_flat_uses_named_boot_file);
    RUN_TEST(test_user_image_prepare_process_rejects_invalid_inputs);
    RUN_TEST(test_user_image_load_copy_rejects_invalid_ranges);
    RUN_TEST(test_user_image_load_flat_rejects_invalid_headers);
    RUN_TEST(test_user_image_load_flat_rejects_noncanonical_header_size);
    RUN_TEST(test_user_image_load_bootfs_flat_rejects_missing_file);
    RUN_TEST(test_user_image_runtime_well_formed_covers_rodata_at_tail);
    RUN_TEST(test_user_image_runtime_short_image_size_drops_tail);
    RUN_TEST(test_user_image_runtime_rejects_image_size_zero);
    RUN_TEST(test_user_image_runtime_rejects_entry_outside_range);
    RUN_TEST(test_user_image_runtime_entry_offset_matches_image_base_plus_offset);
    RUN_TEST(test_user_image_runtime_shipping_shell_image_size);
    RUN_TEST(test_user_image_runtime_shipping_editor_image_size);
    RUN_TEST(test_user_image_runtime_shipping_files_image_size);
    RUN_TEST(test_user_image_runtime_shipping_monitor_image_size);
    RUN_TEST(test_user_image_runtime_shipping_clock_image_size);
    RUN_TEST(test_user_image_runtime_shipping_panel_image_size);
    RUN_TEST(test_user_image_runtime_shipping_control_image_size);
    RUN_TEST(test_user_vm_map_anonymous_installs_user_pte);
    RUN_TEST(test_user_vm_unmap_anonymous_removes_pte_and_frees_pages);
    RUN_TEST(test_user_vm_rejects_invalid_inputs_without_regions);
    RUN_TEST(test_user_vm_map_physical_maps_registered_region);
    RUN_TEST(test_user_vm_map_physical_rejects_unregistered_region);
    RUN_TEST(test_user_vm_physical_mappings_stay_per_process);
    RUN_TEST(test_vfs_mount_and_find_static_node);
    RUN_TEST(test_vfs_read_dispatches_to_node_reader);
    RUN_TEST(test_vfs_rejects_invalid_mounts_and_reads);
    RUN_TEST(test_vfs_mount_static_respects_node_limit);
    RUN_TEST(test_vfs_strip_prefix_returns_suffix_for_exact_prefix);
    RUN_TEST(test_vfs_strip_prefix_rejects_invalid_or_empty_suffix);
    RUN_TEST(test_vfs_open_read_fd_and_close);
    RUN_TEST(test_vfs_open_flags_enforce_read_write_modes);
    RUN_TEST(test_vfs_open_rejects_missing_and_invalid_descriptors);
    RUN_TEST(test_vfs_open_respects_descriptor_limit_and_reuses_closed_slot);
    RUN_TEST(test_vfs_stat_reports_node_size);
    RUN_TEST(test_vfs_list_root_returns_mounted_paths);
    RUN_TEST(test_vfs_list_truncates_to_capacity);
    RUN_TEST(test_vfs_mount_list_validates_paths_and_duplicates);
    RUN_TEST(test_vfs_seek_sets_descriptor_offset);
    RUN_TEST(test_vfs_write_dispatches_to_node_writer);
    RUN_TEST(test_vfs_open_write_fd_updates_offset_and_size);
    RUN_TEST(test_vfs_rejects_overreported_io_counts);
    RUN_TEST(test_vfs_unlink_rejects_non_fat_paths);
    RUN_TEST(test_vfs_rename_rejects_non_fat_paths);
    RUN_TEST(test_tmpfs_create_write_read_stat_and_delete);
    RUN_TEST(test_tmpfs_write_clamps_to_file_capacity);
    RUN_TEST(test_tmpfs_reused_slot_clears_old_file_bytes);
    RUN_TEST(test_tmpfs_rejects_invalid_inputs_and_duplicates);
    RUN_TEST(test_tmpfs_respects_file_limit_and_reuses_deleted_slot);
    RUN_TEST(test_tmpfs_mount_vfs_read_write_and_stat);
    RUN_TEST(test_tmpfs_mount_vfs_rejects_missing_and_invalid_inputs);
    RUN_TEST(test_tmpfs_failed_mount_does_not_consume_slot);
    RUN_TEST(test_fat32_mount_reads_geometry);
    RUN_TEST(test_fat32_default_fs_tracks_successful_mount_only);
    RUN_TEST(test_fat32_open_root_finds_8_3_file);
    RUN_TEST(test_fat32_read_file_across_cluster_chain);
    RUN_TEST(test_fat32_read_clamps_and_honors_offset);
    RUN_TEST(test_fat32_rejects_invalid_boot_sector_and_missing_files);
    RUN_TEST(test_fat32_mount_vfs_file_reads_through_vfs);
    RUN_TEST(test_fat32_list_root_returns_short_names);
    RUN_TEST(test_fat32_mount_vfs_root_lists_through_vfs);
    RUN_TEST(test_fat32_write_overwrites_and_updates_size);
    RUN_TEST(test_fat32_write_grows_within_chain_and_rejects_capacity_overflow);
    RUN_TEST(test_fat32_mount_vfs_file_writes_through_vfs);
    RUN_TEST(test_fat32_vfs_open_mounts_existing_root_file_dynamically);
    RUN_TEST(test_fat32_vfs_open_create_makes_missing_root_file);
    RUN_TEST(test_fat32_vfs_open_create_rejects_bad_flags_and_paths);
    RUN_TEST(test_fat32_create_writes_dir_entry_and_allocates_cluster);
    RUN_TEST(test_fat32_create_then_write_persists_through_open);
    RUN_TEST(test_fat32_create_rejects_duplicate_name);
    RUN_TEST(test_fat32_write_grows_chain_when_capacity_exhausted);
    RUN_TEST(test_fat32_delete_frees_chain_and_removes_entry);
    RUN_TEST(test_fat32_delete_allows_empty_file_without_cluster);
    RUN_TEST(test_fat32_rename_updates_short_name_in_place);
    RUN_TEST(test_fat32_rename_rejects_destination_collision);
    RUN_TEST(test_fat32_write_to_brand_new_file_uses_allocated_cluster);
    RUN_TEST(test_dhcp_options_parse_extracts_known_options);
    RUN_TEST(test_dhcp_options_parse_skips_padding_and_stops_at_end);
    RUN_TEST(test_dhcp_options_parse_skips_magic_cookie_when_present);
    RUN_TEST(test_dhcp_options_parse_rejects_truncated_option);
    RUN_TEST(test_dhcp_options_parse_clears_output_on_error);
    RUN_TEST(test_ipc_send_and_recv_message);
    RUN_TEST(test_ipc_keeps_messages_by_target_pid);
    RUN_TEST(test_ipc_preserves_fifo_after_slot_reuse);
    RUN_TEST(test_ipc_recv_zeroes_unused_payload_tail);
    RUN_TEST(test_ipc_rejects_invalid_inputs_and_full_queue);
    RUN_TEST(test_font_draw_char_renders_8x8_glyph);
    RUN_TEST(test_font_draw_text_advances_and_handles_lowercase);
    RUN_TEST(test_font_draw_text_renders_punctuation);
    RUN_TEST(test_font_draw_char_ignores_non_ascii_bytes);
    RUN_TEST(test_font_draw_text_clips_at_framebuffer_edge);
    RUN_TEST(test_font_draw_text_handles_newline);
    RUN_TEST(test_font_draw_text_clipped_stops_at_max_h);
    RUN_TEST(test_font_draw_text_clipped_handles_zero_max_h);
    RUN_TEST(test_gui_init_for_framebuffer_starts_with_empty_desktop);
    RUN_TEST(test_gui_create_draws_windows_in_creation_order);
    RUN_TEST(test_gui_move_window_redraws_at_new_position);
    RUN_TEST(test_gui_rejects_invalid_inputs_and_window_limit);
    RUN_TEST(test_gui_delivers_key_to_focused_window);
    RUN_TEST(test_gui_cursor_move_clamps_to_bounds);
    RUN_TEST(test_gui_draw_renders_16x16_cursor_over_desktop);
    RUN_TEST(test_gui_left_click_raises_window_under_cursor);
    RUN_TEST(test_gui_left_click_on_background_clears_focus);
    RUN_TEST(test_gui_right_and_middle_buttons_set_mask_without_focus_change);
    RUN_TEST(test_gui_focused_window_gets_brighter_border);
    RUN_TEST(test_gui_drag_moves_window_following_cursor);
    RUN_TEST(test_gui_drag_rejects_unknown_window);
    RUN_TEST(test_gui_draw_fills_vertical_gradient);
    RUN_TEST(test_gui_drag_stays_active_until_left_release);
    RUN_TEST(test_gui_title_bar_shifts_owner_draw_below_bar);
    RUN_TEST(test_gui_title_bar_paints_bar_and_text);
    RUN_TEST(test_gui_title_separator_stays_inside_title_bar);
    RUN_TEST(test_gui_partial_repaint_preserves_title_close_button);
    RUN_TEST(test_gui_cursor_damage_restores_partial_close_icon);
    RUN_TEST(test_gui_partial_title_repaint_does_not_bleed_over_front_window);
    RUN_TEST(test_gui_partial_title_text_repaint_is_clipped_to_damage_rect);
    RUN_TEST(test_gui_cursor_shape_changes_over_clickable_title_decoration);
    RUN_TEST(test_gui_set_cursor_shape_rejects_unknown_shapes);
    RUN_TEST(test_gui_cursor_register_region_installs_shape_inside_window);
    RUN_TEST(test_gui_cursor_register_region_overrides_title_bar_default);
    RUN_TEST(test_gui_cursor_register_region_first_matching_slot_wins);
    RUN_TEST(test_gui_cursor_register_region_delete_clears_slot);
    RUN_TEST(test_gui_cursor_register_region_rejects_invalid_inputs);
    RUN_TEST(test_gui_cursor_register_region_clears_on_window_destroy);
    RUN_TEST(test_gui_cursor_register_region_constant_is_sane);
    RUN_TEST(test_gui_set_title_bar_rejects_height_larger_than_window);
    RUN_TEST(test_gui_set_title_bar_zero_disables_bar);
    RUN_TEST(test_gui_decoration_hit_test_reports_slot_geometry);
    RUN_TEST(test_gui_close_button_pushed_event_on_title_bar_click);
    RUN_TEST(test_gui_close_button_destroys_window_for_dead_owner);
    RUN_TEST(test_gui_close_button_destroys_window_for_reclaimed_owner);
    RUN_TEST(test_gui_close_button_click_outside_box_starts_drag);
    RUN_TEST(test_gui_focus_window_switches_to_target);
    RUN_TEST(test_gui_focus_window_raises_target_above_overlap);
    RUN_TEST(test_gui_window_for_pid_returns_owner_windows);
    RUN_TEST(test_gui_backing_buffer_lazy_allocates_on_first_draw);
    RUN_TEST(test_gui_backing_buffer_init_to_bg_color);
    RUN_TEST(test_gui_backing_buffer_survives_compositor_redraw);
    RUN_TEST(test_gui_backing_buffer_follows_window_on_drag);
    RUN_TEST(test_gui_backing_buffer_freed_on_destroy);
    RUN_TEST(test_gui_backing_buffer_reallocates_when_window_reused);
    RUN_TEST(test_gui_backing_buffer_resize_clears_reused_capacity);
    RUN_TEST(test_gui_damage_init_queues_full_sentinel);
    RUN_TEST(test_gui_damage_add_clips_to_framebuffer);
    RUN_TEST(test_gui_damage_add_merges_overlapping_rects);
    RUN_TEST(test_gui_damage_add_full_collapses_to_sentinel);
    RUN_TEST(test_gui_damage_overflow_collapses_to_sentinel);
    RUN_TEST(test_gui_damage_draw_text_pushes_tight_rect);
    RUN_TEST(test_gui_owner_draw_marks_desktop_dirty_without_flush);
    RUN_TEST(test_gui_damage_draw_rect_pushes_content_rect);
    RUN_TEST(test_gui_damage_title_bar_offset_in_draw_text);
    RUN_TEST(test_gui_damage_move_pushes_union_rect);
    RUN_TEST(test_gui_damage_partial_repaint_leaves_pixels_outside_rect);
    RUN_TEST(test_gui_damage_full_sentinel_re_paints_everything);
    RUN_TEST(test_gui_damage_partial_repaint_paints_every_pixel_in_rect);
    RUN_TEST(test_gui_damage_partial_repaint_skips_when_rect_outside_fb);
    RUN_TEST(test_gui_damage_partial_repaint_with_multiple_disjoint_rects);
    RUN_TEST(test_gui_partial_window_repaint_does_not_overwrite_cursor_outside_damage);
    RUN_TEST(test_gui_minimized_windows_are_not_composited);
    RUN_TEST(test_gui_cursor_is_painted_after_full_window_composition);
    RUN_TEST(test_gui_cursor_is_painted_after_partial_window_composition);
    RUN_TEST(test_input_injects_ascii_byte_directly);
    RUN_TEST(test_input_translates_esc_up_to_key_up);
    RUN_TEST(test_input_translates_esc_down_to_key_down);
    RUN_TEST(test_input_translates_esc_left_and_right);
    RUN_TEST(test_input_keeps_esc_for_unrelated_followup);
    RUN_TEST(test_input_supports_consecutive_esc_sequences);
    RUN_TEST(test_input_translates_ansi_navigation_tilde_sequences);
    RUN_TEST(test_input_keymap_maps_navigation_keys);
    RUN_TEST(test_input_keymap_maps_ctrl_letters);
    RUN_TEST(test_input_queue_init_resets_partial_ansi_sequence);
    RUN_TEST(test_input_poll_char_mask_strips_high_bits);
    RUN_TEST(test_input_peek_returns_event_without_removing);
    RUN_TEST(test_input_peek_returns_minus_one_when_empty);
    RUN_TEST(test_input_peek_then_poll_returns_same_event);
    RUN_TEST(test_virtio_input_probe_rejects_wrong_device);
    RUN_TEST(test_virtio_input_init_records_negotiated_queue_size);
    RUN_TEST(test_virtio_input_rejects_invalid_inputs);
    RUN_TEST(test_virtio_input_key_translation_maps_ascii_and_navigation);
    RUN_TEST(test_virtio_input_key_translation_maps_ctrl_letters);
    RUN_TEST(test_hid_parse_boot_keyboard_decodes_modifiers_and_keys);
    RUN_TEST(test_hid_parse_boot_keyboard_rejects_short_buffer);
    RUN_TEST(test_hid_parse_boot_keyboard_rejects_null);
    RUN_TEST(test_hid_parse_boot_mouse_decodes_buttons_and_motion);
    RUN_TEST(test_hid_parse_boot_mouse_sign_extends_negative_motion);
    RUN_TEST(test_hid_parse_boot_mouse_rejects_short_buffer);
    RUN_TEST(test_hid_usage_to_ascii_lowercase_letters);
    RUN_TEST(test_hid_usage_to_ascii_uppercase_letters_with_shift);
    RUN_TEST(test_hid_usage_to_ascii_digits_and_punctuation);
    RUN_TEST(test_hid_usage_to_ascii_shifted_punctuation);
    RUN_TEST(test_hid_usage_to_ascii_returns_zero_for_modifiers);
    RUN_TEST(test_hid_usage_to_ascii_returns_zero_for_function_keys);
    RUN_TEST(test_hid_usage_to_ascii_high_usage_ids_return_zero);
    RUN_TEST(test_hid_usage_to_input_key_maps_navigation);
    RUN_TEST(test_hid_usage_to_input_key_maps_ctrl_letters);
    RUN_TEST(test_hid_keyboard_report_emits_press_for_new_key);
    RUN_TEST(test_hid_keyboard_report_emits_navigation_key);
    RUN_TEST(test_hid_keyboard_report_emits_ctrl_s);
    RUN_TEST(test_hid_keyboard_report_emits_release_when_key_dropped);
    RUN_TEST(test_hid_keyboard_report_filters_single_empty_repeat_gap);
    RUN_TEST(test_hid_keyboard_report_handles_two_key_rollover);
    RUN_TEST(test_hid_mouse_report_emits_move_when_deltas_nonzero);
    RUN_TEST(test_hid_mouse_report_emits_button_press_and_release);
    RUN_TEST(test_hid_mouse_report_emits_no_events_when_idle);
    RUN_TEST(test_hid_mouse_report_rejects_null_arguments);
    RUN_TEST(test_pci_bar_address_extracts_32bit_mmio);
    RUN_TEST(test_pci_bar_address_ignores_io_space_bar);
    RUN_TEST(test_pci_bar_address_combines_64bit_pair);
    RUN_TEST(test_pci_bar_address_low_32bit_ignores_next_bar);
    RUN_TEST(test_usb_walk_configuration_finds_interface);
    RUN_TEST(test_usb_walk_configuration_attaches_endpoint);
    RUN_TEST(test_usb_walk_configuration_attaches_hid_class_descriptor);
    RUN_TEST(test_usb_walk_configuration_rejects_short_buffer);
    RUN_TEST(test_usb_walk_configuration_rejects_wrong_descriptor_type);
    RUN_TEST(test_usb_walk_configuration_rejects_short_typed_descriptors);
    RUN_TEST(test_usb_find_interface_returns_null_when_no_match);
    RUN_TEST(test_usb_find_interface_matches_exact_class_subclass_protocol);
    RUN_TEST(test_usb_find_endpoint_in_returns_interrupt_endpoint);
    RUN_TEST(test_usb_find_endpoint_in_returns_null_when_too_small);
    RUN_TEST(test_pci_assign_bars_skips_already_programmed_bars);
    RUN_TEST(test_pci_assign_bars_function_pointer_exists);
    RUN_TEST(test_hid_state_reset_clears_count);
    RUN_TEST(test_hid_state_reset_clears_device_endpoints);
    RUN_TEST(test_hid_poll_all_returns_zero_on_empty_state);
    RUN_TEST(test_hid_state_count_field_is_accessible);
    RUN_TEST(test_hid_max_devices_constant_is_sane);
    RUN_TEST(test_hid_init_registers_mouse_protocol);
    RUN_TEST(test_hid_init_zeroes_prev_keys_for_new_device);
    RUN_TEST(test_hid_add_device_appends_and_copies_usb_handle);
    RUN_TEST(test_hid_init_returns_zero_when_no_boot_interfaces);
    RUN_TEST(test_hid_init_returns_zero_for_null_state);
    RUN_TEST(test_hid_init_keeps_max_four_devices);
    RUN_TEST(test_fb_init_rejects_invalid_geometry);
    RUN_TEST(test_fb_putpixel_and_fillrect_clip_to_bounds);
    RUN_TEST(test_fb_blit_copies_and_clips_source_pixels);
    RUN_TEST(test_fb_draw_line_rect_circle_and_alpha);
    RUN_TEST(test_fb_draw_rect_clips_huge_dimensions_without_wrapping);
    RUN_TEST(test_dtb_get_framebuffer_reads_simple_framebuffer);
    RUN_TEST(test_dtb_get_framebuffer_rejects_incomplete_node);
    RUN_TEST(test_dtb_get_framebuffer_rejects_header_offsets_outside_blob);
    RUN_TEST(test_virtio_blk_probe_reads_capacity);
    RUN_TEST(test_virtio_blk_probe_rejects_non_block_device);
    RUN_TEST(test_virtio_blk_probe_range_finds_later_transport);
    RUN_TEST(test_virtio_blk_init_programs_modern_queue);
    RUN_TEST(test_virtio_net_probe_reads_mac_address);
    RUN_TEST(test_virtio_net_init_negotiates_feature_banks);
    RUN_TEST(test_virtio_net_tx_uses_single_frame_buffer);
    RUN_TEST(test_virtio_net_ready_ops_reject_invalid_device_state);
    RUN_TEST(test_syscall_abi_implemented_numbers_match_dispatch);
    RUN_TEST(test_syscall_abi_ranges_do_not_overlap);
    RUN_TEST(test_syscall_abi_error_codes_match_documented_constants);
    RUN_TEST(test_syscall_abi_vfs_open_flags_match_documentation);
    RUN_TEST(test_syscall_abi_user_exit_codes_match_documented_constants);
    RUN_TEST(test_syscall_abi_user_range_validation_rejects_out_of_region);
    RUN_TEST(test_syscall_helpers_user_buffers_validate_registered_ranges);
    RUN_TEST(test_syscall_helpers_copy_cstr_validates_each_byte);
    RUN_TEST(test_syscall_helpers_copy_cstr_rejects_unregistered_tail);
    RUN_TEST(test_syscall_helpers_owner_window_error_modes_are_stable);
    RUN_TEST(test_process_isolation_two_processes_have_independent_regions);
    RUN_TEST(test_process_isolation_mmap_returns_distinct_addresses);
    RUN_TEST(test_process_isolation_mmap_rejects_hint_nonzero);
    RUN_TEST(test_process_isolation_mmap_rejects_unknown_flags);
    RUN_TEST(test_process_isolation_zero_process_returns_false);
    RUN_TEST(test_window_abi_event_layout_is_12_bytes);
    RUN_TEST(test_window_abi_event_types_match_documented_numbers);
    RUN_TEST(test_window_abi_create_window_for_pid_assigns_owner);
    RUN_TEST(test_window_abi_focus_window_raises_z_order);
    RUN_TEST(test_window_abi_focus_no_focus_window_rejected);
    RUN_TEST(test_window_abi_window_for_pid_skips_no_owner);
    RUN_TEST(test_window_abi_window_for_pid_skips_skip_taskbar);
    RUN_TEST(test_window_abi_get_bounds_returns_current_geometry);
    RUN_TEST(test_window_abi_resize_window_updates_geometry_and_queues_event);
    RUN_TEST(test_window_abi_resize_window_rejects_out_of_bounds);
    RUN_TEST(test_window_abi_event_types_includes_minimize_and_maximize);
    RUN_TEST(test_window_abi_minimize_hides_window_and_queues_event);

    return UNITY_END();
}
